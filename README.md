...
- **임시저장 / 확인 후 게시**: "임시저장"은 관리자 화면에만 남고 학생
  화면에는 절대 반영되지 않습니다(초안 확인용). 내용을 다 확인했으면
  **"확인 후 게시"**를 눌러야 학생 화면에 반영됩니다 — 이때 한글 메뉴명을
  5개 언어(중국어/베트남어/영어/몽골어)로 자동 번역하고, 이미지를 새로
  올렸다면 Firebase Storage에 업로드합니다. 같은 메뉴명은 `menuTranslations`
  사전에 캐시되어 다음 주에 다시 번역하지 않습니다. 번역이 없는 메뉴는
  학생 화면에서 자동으로 한국어로 대체 표시됩니다(빈 칸이나 "undefined"가
  뜨지 않음).
...
- `js/weekly-menu-sync.js` : 관리자가 "확인 후 게시"한 주간메뉴(published 상태만)를
