...
카테고리는 친구 만들기·중고거래·도움 요청·같이 해요·생활정보 5개이며,
게시글은 등록 시 5개 언어로 자동 번역되고 댓글은 "번역 보기"를 눌렀을
때만 필요한 언어 1개를 번역해 비용을 아낍니다.
...
(짧은 번역 전용 저비용 모델 `gpt-4o-mini` 사용, 게시글은 등록/수정 시
1회만, 댓글은 "번역 보기" 클릭 시 필요한 언어 1개만, 같은 조합은
재번역하지 않고 Firestore에 캐시된 결과를 재사용합니다).
