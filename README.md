...
`/community`로 이동합니다. 회원가입·이메일 인증을 마친 회원만 게시판을
볼 수 있고, 비회원이나 이메일 미인증 회원은 로그인 안내 화면만 보게
됩니다. 카테고리는 친구 만들기·중고거래·도움 요청·같이 해요·생활정보
5개이며, 게시글은 등록 시 5개 언어로 자동 번역되고 댓글은 "번역 보기"를
눌렀을 때만 필요한 언어 1개를 번역해 비용을 아낍니다.

### 11-1. 배포 전에 꼭 해야 하는 일(Firestore/Storage 규칙)
...
관리자 로그인과 동일 설정) 회원가입·이메일 인증은 추가 설정 없이 바로
동작합니다.
...
1. 관리자로 지정할 사람이 `/community/signup`에서 먼저 정상적으로
   회원가입하고 이메일 인증까지 마칩니다.
