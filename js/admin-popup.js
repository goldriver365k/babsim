...
var AdminPopup = (function () {
  ...
  var pendingCompressPromise = null; // 방금 고른 파일의 압축 결과(Promise<Blob|null>)

  // 팝업 이미지 속도 2단계: 업로드 전 브라우저에서 800×1000 이내로 축소
  // 합니다(원본 비율 유지). 4:5(800×1000) 권장 규격과 맞춘 값입니다.
  var POPUP_MAX_WIDTH = 800;
  var POPUP_MAX_HEIGHT = 1000;
  var POPUP_PHOTO_QUALITY = 0.8; // 사진 계열(투명 없음) 압축 품질
  var POPUP_WARN_BYTES = 500 * 1024; // 이 이상이면 업로드는 막지 않고 경고만
  ...
  // 업로드 전 이미지 로딩(커뮤니티 게시글 사진 압축, js/community.js
  // compressImage와 같은 Canvas API 방식 재사용 — 새 라이브러리 없음).
  function loadPopupImageSource(file) {
    if (window.createImageBitmap) {
      return createImageBitmap(file, { imageOrientation: "from-image" }).catch(function () {
        return loadPopupImageViaTag(file);
      });
    }
    return loadPopupImageViaTag(file);
  }
  function loadPopupImageViaTag(file) {
  ...
  // 캔버스에 투명 픽셀이 하나라도 있으면 PNG로 유지(무조건 JPEG로 바꿔
  // 투명 배경을 깨뜨리지 않기 위함). 확인 자체가 실패하면 안전하게
  // 투명으로 간주합니다.
  function popupCanvasHasAlpha(canvas) {
    try {
  ...
  // 업로드 전 브라우저에서 800×1000 이내로 축소(원본 비율 유지) + 압축합니다.
  // 투명 배경이 있으면 PNG로, 없으면 WebP 품질 0.8로 저장합니다. 실패하면
  // null을 돌려주고 호출부에서 원본 파일을 그대로 올립니다(업로드를
  // 막지 않음 — 구조가 복잡해지거나 위험하면 압축을 강행하지 않음).
  function compressPopupImage(file) {
    return loadPopupImageSource(file).then(function (source) {
      var srcW = source.width, srcH = source.height;
      var scale = Math.min(1, POPUP_MAX_WIDTH / srcW, POPUP_MAX_HEIGHT / srcH);
      ...
      var keepPng = popupCanvasHasAlpha(canvas);
      return new Promise(function (resolve, reject) {
        canvas.toBlob(function (blob) {
          if (blob) resolve(blob); else reject(new Error("COMPRESS_FAILED"));
        }, keepPng ? "image/png" : "image/webp", keepPng ? undefined : POPUP_PHOTO_QUALITY);
      });
  ...
  function uploadPopupImage(blob, fallbackName) {
    var st = storage();
  ...
  function warnIfLarge(size) {
    els.imageError.textContent = (typeof size === "number" && size > POPUP_WARN_BYTES)
      ? "이미지 용량이 커서 팝업 표시가 느려질 수 있습니다."
      : "";
  }
  ...
  function handleImagePick() {
    ...
    pendingCompressPromise = compressPopupImage(file).then(function (blob) {
  ...
  function handleSave(e) {
    ...
    var uploadPromise = !file
      ...
      : Promise.resolve(pendingCompressPromise || compressPopupImage(file)).then(function (blob) {
          return uploadPopupImage(blob || file, file.name);
        });
