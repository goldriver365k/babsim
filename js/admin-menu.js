...
var AdminMenu = (function () {
  ...
  var editingOriginalDescription = null; // 수정 중인 메뉴의 기존 description(문자열 또는 다국어 객체, 보존용)
  var editingOriginalDescriptionKo = ""; // 위 값의 한국어만 — 변경 여부 판단(재번역 필요성)용
  var TRANSLATE_TARGET_LANGS = ["zh", "vi", "en", "mn", "bn", "my"];
  ...
  // name/description은 다국어 객체({ko:"...", en:"...", ...})로 저장되지만,
  // 기존(번역 전) 데이터는 description이 아직 순수 문자열일 수 있습니다 —
  // 관리 화면은 항상 한국어만 다루므로 두 경우 모두 한국어 값만 꺼냅니다.
  function descriptionKoOf(m) {
    if (!m || !m.description) return "";
    return typeof m.description === "object" ? (m.description.ko || "") : m.description;
  }
  ...
  function startEdit(id, m) {
    ...
    editingOriginalDescription = m.description || null;
    editingOriginalDescriptionKo = descriptionKoOf(m);
    pendingCompressPromise = null;
    els.nameInput.value = (m.name && m.name.ko) || "";
    els.descriptionInput.value = editingOriginalDescriptionKo;
  ...
  /* ---------------- 메뉴명/설명 번역(기존 커뮤니티 번역 서버 함수 재사용) ----------------
     새 Netlify Function/새 API를 만들지 않고, 커뮤니티 게시글 번역에 이미
     쓰이는 netlify/functions/community-translate.js의 translatePost
     액션을 그대로 재사용합니다(이미 로그인된 관리자 계정의 idToken 사용).
     메뉴명(title)+메뉴설명(content)을 한 번의 호출로 6개 언어 모두
     번역합니다 — 메뉴마다 여러 번 호출하지 않습니다. 설명이 비어 있으면
     content 필수 조건을 맞추기 위해 메뉴명을 대신 넣어 호출하되, 그
     번역 결과는 저장하지 않습니다(설명 없음 상태 유지). 실패하면 null을
     돌려주고 호출부가 기존 한국어만 저장하는 방식으로 안전하게 돌아갑니다.
     ========================================================================== */
  // 항상 { ok: true, name, description } 또는 { ok: false, reason: "..." }를
  // 돌려줍니다 — 실패 이유를 handleSave가 저장 완료 메시지에 그대로
  // 붙여서 보여줄 수 있도록(브라우저 개발자도구 없이도 관리자가 원인을
  // 바로 확인 가능).
  function translateMenuText(nameKo, descriptionKo) {
    var user = currentAdminUser();
    if (!user) return Promise.resolve({ ok: false, reason: "관리자 로그인 정보 없음" });
    return user.getIdToken().then(function (idToken) {
      return fetch("/.netlify/functions/community-translate", {
        ...
        body: JSON.stringify({
          ...
          originalLanguage: "ko",
          title: nameKo,
          content: descriptionKo || nameKo,
          targetLanguages: TRANSLATE_TARGET_LANGS
        })
      }).then(function (res) {
        if (!res.ok) {
          return res.json().catch(function () { return null; }).then(function (errBody) {
            return { ok: false, reason: "서버 응답 " + res.status + (errBody && errBody.error ? "(" + errBody.error + ")" : "") };
          });
        }
      ...
      if (!data || !data.translations) return { ok: false, reason: "번역 결과 형식 오류" };
      var nameOut = { ko: nameKo };
      var descOut = descriptionKo ? { ko: descriptionKo } : null;
      var translatedCount = 0;
      TRANSLATE_TARGET_LANGS.forEach(function (lang) {
        var tr = data.translations[lang];
        // AI가 가끔 번역 대신 원문을 그대로 돌려줄 때가 있습니다 — 한국어
        // 원문과 글자까지 완전히 같으면(다른 언어 문자 체계상 정상 번역이면
        // 거의 있을 수 없는 일) 번역 실패로 간주해 저장하지 않습니다(다음
        // 저장 때 다시 시도되도록).
        if (tr && typeof tr.title === "string" && tr.title.trim() && tr.title !== nameKo) {
          nameOut[lang] = tr.title;
          translatedCount++;
        }
        if (descriptionKo && tr && typeof tr.content === "string" && tr.content.trim() && tr.content !== descriptionKo) {
          descOut[lang] = tr.content;
        }
      });
      if (!translatedCount) return { ok: false, reason: "번역된 언어 0개(AI가 원문을 그대로 반환)" };
      return { ok: true, name: nameOut, description: descOut };
    }).catch(function (e) {
      return { ok: false, reason: "요청 실패(" + (e && e.message ? e.message : "네트워크 오류") + ")" };
    });
  }
  ...
  function handleSave(e) {
    ...
    var lastTranslateNote = ""; // 번역 성공/실패 메모 — 저장 완료 메시지에 그대로 붙여 보여줌
    ...
    uploadPromise.then(function (newUrl) {
      ...
      // 메뉴명/설명 텍스트가 바뀌었거나, 아직 다국어 번역이 채워지지 않은
      // (예: 번역 기능이 생기기 전에 등록된) 기존 메뉴라면 번역을
      // 다시 요청합니다. 이미 7개 언어가 모두 채워진 메뉴를 가격/이미지만
      // 바꿔 저장할 때는 번역 API를 부르지 않습니다(반복 호출 방지).
      // AI가 번역 대신 원문을 그대로 돌려준 적이 있으면(한국어와 글자가
      // 완전히 같음) 언어 칸이 채워져 있어도 "아직 번역 안 됨"으로 보고
      // 다음 저장 때 다시 시도합니다.
      var hasFullNameTranslation = !!(editingOriginalName && TRANSLATE_TARGET_LANGS.every(function (l) {
        var v = editingOriginalName[l];
        return typeof v === "string" && v && v !== editingOriginalName.ko;
      }));
      var hasFullDescriptionTranslation = !editingOriginalDescriptionKo || !!(
        editingOriginalDescription && typeof editingOriginalDescription === "object" &&
        TRANSLATE_TARGET_LANGS.every(function (l) {
          var v = editingOriginalDescription[l];
          return typeof v === "string" && v && v !== editingOriginalDescription.ko;
        })
      );
      var needsTranslate = !editingId || !hasFullNameTranslation || !hasFullDescriptionTranslation ||
        name !== ((editingOriginalName && editingOriginalName.ko) || "") ||
        description !== editingOriginalDescriptionKo;
      ...
      return translatedPromise.then(function (translateResult) {
        ...
        if (editingId) {
          // 기존(다국어 번역이 있을 수 있는) name 객체에 새 번역(또는
          // 최소한 한국어)만 덮어써서 저장합니다 — 통째로 교체하면 기존
          // 메뉴 연결로 들어온 다른 언어 번역이 사라집니다.
          var nextName = Object.assign({}, editingOriginalName, (translated && translated.name) || { ko: name });
          // 설명을 지우면 필드 자체를 제거합니다(빈 문자열로 남겨 화면에
          // 빈 줄이 생기지 않도록). 새로 번역되면 그 결과를, 아니면(가격만
          // 수정 등) 기존 다국어 설명 객체에 한국어만 갱신해 다른 언어
          // 번역이 사라지지 않게 합니다(레거시 문자열 데이터는 그대로 유지).
          var nextDescription;
          if (!description) {
            nextDescription = firebase.firestore.FieldValue.delete();
          } else if (translated && translated.description) {
            nextDescription = translated.description;
          } else if (editingOriginalDescription && typeof editingOriginalDescription === "object") {
            nextDescription = Object.assign({}, editingOriginalDescription, { ko: description });
          } else {
        ...
        var newDoc = {
          store: currentStore,
          name: (translated && translated.name) || { ko: name },
          price: priceNum,
          image: imageUrl,
          soldOut: false,
          isActive: true,
        ...
        if (description) newDoc.description = (translated && translated.description) || description; // 비어 있으면 필드 자체를 만들지 않음
        return d.collection("storeMenus").add(newDoc);
      });
    }).then(function () {
      // resetForm()이 내부적으로 setStatus("")를 호출해 메시지를 곧바로
      // 숨기므로, 반드시 resetForm() 이후에 저장 완료 메시지를 다시
      // 띄워야 실제로 화면에 보입니다(이전에는 매번 순간적으로 가려짐).
      var wasEdit = !!editingId;
      resetForm();
      setStatus((wasEdit ? "수정되었습니다." : "등록되었습니다.") + lastTranslateNote, !!lastTranslateNote);
      loadMenus();
    }).catch(function (err) {
  ...
  function renderMenuRow(tbody, m) {
    ...
    descClamp.textContent = descriptionKoOf(m) || "-";
