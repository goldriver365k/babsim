...
(function () {
  ...
  var els = {};
  var breakfastViewTracked = false; // GA4 breakfast_view(6단계) 중복 방지 — 페이지 세션당 1회만
  ...
  // GA4 홈 서비스 카드 클릭 분석(3단계) — 화면 언어와 무관하게 영문 고정값만
  // 보냅니다. 기존 이동 동작(goToStore/커뮤니티 탭 클릭)은 그대로 두고
  // 클릭 시점에 이벤트만 추가로 보냅니다.
  var HOME_SERVICE_GA_NAME = { bapsim: "babsim", mangwon: "mangwon", hururuk: "hururuk" };
  function trackServiceClick(serviceName) {
    if (typeof gtag === "function") gtag("event", "service_click", { service_name: serviceName });
  }
  ...
  function renderBreakfastArea() {
    ...
    // GA4 breakfast_view(6단계) — 언어 전환·자정 갱신 등으로 이 화면이
    // 다시 그려질 때마다 보내지 않도록 페이지 세션당 1회만 전송합니다.
    if (!breakfastViewTracked) {
      breakfastViewTracked = true;
      if (typeof gtag === "function") gtag("event", "breakfast_view");
    }
    var lang = state.lang;
  ...
  // 완료 처리도 되지 않습니다.
  // GA4 breakfast_rating_complete(6단계)도 같은 이유로 여기서만 전송하고,
  // rating(1~5)만 보낼 뿐 UID·이메일·닉네임 등 개인 식별정보는 넣지 않습니다.
  function handleBreakfastRatingSaved(dateKey, score) {
    markBreakfastRatingCompletedToday();
    closeBreakfastPopup();
    if (typeof gtag === "function") gtag("event", "breakfast_rating_complete", { rating: score });
  }
  ...
  function init() {
    ...
    Array.prototype.forEach.call(document.querySelectorAll(".home-service-card[data-store]"), function (card) {
      ...
      card.addEventListener("click", function () {
        trackServiceClick(HOME_SERVICE_GA_NAME[store] || store);
        goToStore(store);
      });
    });
